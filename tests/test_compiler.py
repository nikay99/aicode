...
class TestCompilerVariables(unittest.TestCase):
    ...
    def test_let_statement(self):
        """Test compiling let statement at top level"""
        program = parse("let x = 42")
        compiler = BytecodeCompiler()
        module = compiler.compile_program(program)

        main_func = module.functions[module.entry_point]
        # Top-level variables use STORE_GLOBAL
        self.assertTrue(any(i.opcode == OpCode.STORE_GLOBAL for i in main_func.code))
...
"""
        ...
        main_func = module.functions[module.entry_point]
        # Should have LOAD_GLOBAL for top-level variables
        self.assertTrue(any(i.opcode == OpCode.LOAD_GLOBAL for i in main_func.code))
...
"""
        ...
        main_func = module.functions[module.entry_point]
        # Should have STORE_GLOBAL for top-level variables
        store_count = sum(1 for i in main_func.code if i.opcode == OpCode.STORE_GLOBAL)
